...
qog<-read_csv("data/qog_std_ts_jan19.csv") %>%
  select(year, ccode, cname, wdi_gerp, wdi_nerp, wdi_gerpf, wdi_nerpf, ccodecow, ccodealp, wdi_expedu,
         bl_asyf, bl_lpf, bl_lpmf, gea_ea1524f, gea_ea1524m, wdi_expedup, ht_region, gle_cgdpc, gle_rgdpc, bl_asymf)

fig2a<-qog %>%
  filter(ht_region==2) %>%
  group_by(year) %>%
  summarise(ave = mean(wdi_gerp,na.rm = TRUE),
            min = min(wdi_gerp,na.rm = TRUE)) %>%
...
data_global<-qog %>%
  filter(ht_region!=2) %>%
  group_by(year) %>%
  summarise(ave = mean(wdi_nerp,na.rm = TRUE)) %>%
  mutate(type="global")
...
```{r, fig3_dataload, include=FALSE, cache=F}
#Packages
...
fig3a<-qog %>%
  ...
  ggplot(aes(x=year, y=y, group=type, color=type, linetype=type)) +
    ...
    theme(legend.title = element_blank(),
          legend.position = c(0.8, 0.2))+
      labs(title="Female Primary School Enrollment Rates",
         x="Year",
         y="Gross Share (%)")
...
fig3b<-qog %>%
  filter(ht_region==2) %>%
  group_by(year) %>%
  summarise(ave = mean(bl_asyf,na.rm = F),
            min = min(bl_asyf,na.rm = F)) %>%
  pivot_longer(-year, names_to = "type", values_to = "y") %>%
  ggplot(aes(x=year, y=y, group=type, color=type, shape=type)) +
    scale_color_discrete(name="Line",
                       breaks = c("ave", "min"),
                       labels = c("Average", "Minimum")) +
      scale_shape_discrete(name="Line",
                       breaks = c("ave", "min"),
                       labels = c("Average", "Minimum")) +
    geom_point() +
    xlim(1970,2000) +
...
```{r, plotfig3, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap = "Enrollment and Completion Rates in Latin America", fig.width=8, fig.height=4, cache=F}
...
```{r, fig4_dataload, include=FALSE, cache=F}

fig4a<-ggplot(qog, aes(gle_cgdpc, wdi_gerp)) + 
geom_jitter(subset(qog,ht_region==2), 
                       mapping = aes(gle_cgdpc, wdi_gerp), alpha =.2) + 
            geom_smooth(se=F)+
            geom_smooth(subset(qog,ht_region==3|
                                 ht_region==4|ht_region==7|ht_region==8|ht_region==10),
                       mapping = aes(gle_cgdpc, wdi_gerp), color="red", se=F) +
              labs(y="Primary (% Gross)",
                 x="GDP per capita",
                 title = "GDP and Enrollment Rates") +
            coord_cartesian(xlim = c(0,20000), expand = TRUE,
                        default = FALSE, clip = "on") +
            theme_bw()

fig4b<-ggplot(qog, aes(gle_cgdpc, bl_asymf)) + 
geom_jitter(subset(qog,ht_region==2), 
                       mapping = aes(gle_cgdpc, bl_asymf), alpha =.2) + 
            geom_smooth(se=F)+
            geom_smooth(subset(qog,ht_region==3|
                                 ht_region==4|ht_region==7|ht_region==8|ht_region==10),
                       mapping = aes(gle_cgdpc, bl_asymf), color="red", se=F) +
              labs(y="Years",
                 x="GDP per capita",
...
fig4c<-ggplot(qog, aes(gle_cgdpc, bl_asyf)) + 
            geom_jitter(subset(qog,ht_region==2), 
                       mapping = aes(gle_cgdpc, bl_asyf), alpha = .2) + 
            geom_smooth(se=F)+
            geom_smooth(subset(qog,ht_region==3|
                                 ht_region==4|ht_region==7|ht_region==8|ht_region==10),
                       mapping = aes(gle_cgdpc, bl_asyf), color="red", se=F) +
              labs(y="Years",
                 x="GDP per capita",
                 title = "Female Years of Schooling") +
            coord_cartesian(xlim = c(0,20000), expand = TRUE,
                        default = FALSE, clip = "on") +
              theme_bw()
...
```{r, plotfig4, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap = "Comparative Performance of Latin American Education Systems", fig.width=8, fig.height=8, cache=F}
grid.arrange(fig4a,fig4b,fig4c, ncol=2)
```
...
```{r, fig5_dataload, include=FALSE, cache=F}
repeaters<-read_csv("data/repeaters.csv")
fig5<-ggplot(repeaters, aes(y=repeaters, x=year)) +
  geom_smooth() +
  geom_jitter(alpha=.2) +
                theme_bw() +
              labs(y="Repeaters (% Enrollment)",
                 x="Years",
                 caption = "Source: UNESCO 2018.",
                 title = "Primary School Repeaters")
```

```{r, plotfig5, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap = "Grade Repetition Rates in Latin America", fig.width=4, fig.height=3.5, cache=F}
fig5
```
...
```{r, fig6_dataload, include=FALSE, cache=F}
#Bivariate Regressions

#Packages
library(binsreg); library(ggplot2); library(vtable)

#Read data
data.reg<-read.csv("data/mini_dataset.csv")
data.reg$timms<-as.factor(data.reg$timms)

library(ggrepel)

p1<-ggplot(data.reg, aes(y=reform_initiatives, x=enrollment_gross))
p3<-ggplot(data.reg, aes(y=reform_initiatives, x=av_schoolyrs))
p4<-ggplot(data.reg, aes(y=reform_initiatives, x=share_schoolf))
p5<-ggplot(data.reg, aes(y=reform_initiatives, x=timms))
...
p1<-p1+geom_point()+
  ...
  geom_text_repel(data.reg,mapping=aes(label=short_name)) +
                  theme_bw() +
              labs(y="# Reforms",
                 x="Enrollment (% Gross)",
...
p3<-p3+geom_point()+
  ...
  geom_smooth(method="lm", se=F)+
  geom_smooth(method="loess", se=F, color="red")+
  labs(x="Average School Years", y="Number of Reforms")+
  geom_text_repel(data.reg,mapping=aes(label=short_name)) +
                  theme_bw() +
              labs(y="# Reforms",
                 x="Years",
                 title = "Average School Years") 
...
p5<-p5+geom_point()+
  ...
  geom_text_repel(data.reg,mapping=aes(label=short_name)) +
              ...
              labs(y="# Reforms",
                 x="TIMMS",
                 title = "Participation in TIMMS") 
```

```{r, plotfig6, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap = "Regressing Education Reform on Various Covariates", fig.width=8, fig.height=8, cache=F}
grid.arrange(p1, p3, p5, p4, ncol=2)
